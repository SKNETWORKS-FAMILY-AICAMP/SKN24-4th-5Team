    (function () {
        ...
        const descriptionCard = document.getElementById('mode-description-card');
        ...
        const fallbackModeDescriptions = {
            practice: {
                title: '📝 연습 모드',
                text: '연습 모드는 시간 제한 없이 질문의 개수를 선택하여 진행됩니다. 질문은 텍스트와 음성으로 제공되며, 회화 연습이나 스피킹 감각을 익히려는 분들께 추천드립니다.'
            },
            real: {
                title: '🎙️ 실전 모드',
                text: '실전 모드는 7분 타이머가 시작되고 비자 인터뷰 질문이 자동으로 출력됩니다. 실제 면접처럼 답변을 음성으로 녹음하며 진행할 수 있습니다.'
            }
        };
        const modeDescriptions = {
            ...fallbackModeDescriptions,
            ...(pageData.modeDescriptions || {})
        };
        ...
        function setMode(mode) {
            ...
            descriptionCard.innerHTML = '<h3></h3><p></p>';
            descriptionCard.querySelector('h3').textContent = modeDescriptions[mode].title;
            descriptionCard.querySelector('p').textContent = modeDescriptions[mode].text;
        }
        ...
        function setUploadedFile(file) {
            if (!validateFile(file)) {
                fileInput.value = '';
                uploadedFile = null;
                fileNameDisplay.textContent = '파일 이름.pdf';
                return;
            }
        ...
        modeButtons.forEach((button) => {
            ...
            button.addEventListener('click', () => {
                if (!uploadCompleted) {
                    openUploadModal(button.dataset.modeButton);
                    return;
                }

                showInterviewMode(button.dataset.modeButton);
            });
        });
