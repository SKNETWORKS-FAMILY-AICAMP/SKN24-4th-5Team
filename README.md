...
-  2023년 기준 해외 유학생의 약 79%가 정규 학위 취득을 목적으로 체류 중이며, 이는 단순 경험보다 '확실한 결과'를 원하는 고관여 사용자가 늘어났음을 의미함
	- 👉**본 프로젝트는 단순 정보 탐색을 넘어 학위 취득과 비자 승인이라는 명확한 생존형 목적을 가진 '결과 중심의 학위 유학층'을 핵심 타겟으로 설정**
...
### **[Service 1] Broad Perspective: 넓은 시야**
...
-   **멀티모달 AI 비자 시뮬레이션**: **STT/TTS** 기반의 '실전/연습 모드'를 통해 35%의 비자 거절 불확실성을 정면 돌파함
    ...
    -   **[한계 극복]** 
	    - 주관적 판단에 의존하던 기존 방식을 RAG 기반 정보 일치성 검토, 언어 문법 및 음성 분석(속도·유창성 등)의 다각도 평가
...
>목표: Django와 AWS의 견고한 인프라로 유학 준비의 심리적 장벽을 허물고 도전을 응원하다_

-   **가치 지향형 고성능 시스템**: **Django와 AWS(RDS)** 기반의 안정적인 인프라 위에서 복잡한 절차를 혁신적인 UX로 풀어내어 준비생의 심리적 피로를 해소함
    
    -   **[한계 극복]** 
	    - 정보 분산으로 인한 번아웃과 불투명한 시장의 고비용·저효율 구조를 탄탄한 클라우드 기술로 해결하여, 사용자가 오직 자신의 꿈에만 집중할 수 있도록 강력하게 서포트함
