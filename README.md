...
### **[3] Unified Infrastructure: 견고한 기술력이 뒷받침하는 차세대 유학 솔루션**

>목표: Django와 AWS RDS를 활용해 분산된 입시 정보와 비자 인터뷰 연습을 하나의 시스템으로 통합함

-   **가치 지향형 고성능 시스템**: 파편화된 전 세계 대학 데이터에 대한 챗봇 시스템 구축하고, 모의 비자 인터뷰와 연결하여 정보 탐색부터 실전 대비까지 한 곳에서 해결하는 유기적인 구조를 구현함
    
    -   **[한계 극복]** 
	    - 단절된 서비스 통합: 학교 정보는 대학 사이트에서, 비자 준비는 개별 유학원에서 따로 해야 했던 서비스 간의 단절을 없애고 준비 시간을 획기적으로 단축함
     	- 준비 과정의 명확성: 불투명한 정보 시장에서 겪는 혼란을 줄이고, '데이터 기반 정보 확인 → AI 면접 연습'으로 이어지는 직관적인 준비 경로를 제공하여 합격 가능성을 높임
