...
- 국내 대학(연세대 등)은 '통합자료실'을 통해 모든 입시 정보를 단일 PDF로 제공하는 반면, UIUC를 비롯한 해외 대학은 정보가 여러 웹페이지의 분리 및 500~7000 페이지 분량의 입시요강에 파편화되어 있음
  _ 👉 이러한 분산된 구조는 유학원 등 특정 서비스 공급자에게 정보 권력이 독점되는 결과를 초래하며, 학생은 고가의 컨설팅 비용을 지불해야만 하는 구조적 비효율에 직면함
  _ 👉 따라서 분산된 입시 데이터를 하나로 통합하고, 복잡한 준비 과정을 일괄적으로 관리해 줄 수 있는 통합 솔루션(챗봇)의 도입이 시급함
  |
...
| 항목            | Google TTS | Azure TTS | Coqui TTS | gTTS ⭐ | ElevenLabs |
| --------------- | ---------- | --------- | --------- | ------- | ---------- |
| 오프라인 동작   | ❌         | ❌        | ✔         | ❌      | ❌         |
| 무료/오픈소스   | ❌         | ❌        | ✔         | ✔       | ❌         |
| 설치 난이도     | 쉬움       | 중간      | 어려움    | 쉬움    | 쉬움       |
| 음성 자연스러움 | 높음       | 매우 높음 | 높음      | 보통    | 매우 높음  |
| Python 연동     | ✔          | ✔         | ✔         | ✔       | ✔          |
| API 비용        | 유료       | 유료      | 없음      | 없음    | 유료       |
| **응답 속도**   | 매우 빠름  | 매우 빠름 | 중간      | 빠름    | 빠름       |
...
- FastAPI에서 구동되는 LLM이 RDS를 활용하여 채팅 메시지 히스토리를 조회한 후, 응답을 생성하기 위한 컨텍스트로 사용됨
...
<img src="./image/화면설계서1.png" />
<img src="./image/화면설계서2.png" />
<img src="./image/화면설계서3.png" />
<img src="./image/화면설계서4.png" />
...
- [Solution] 유학 준비의 End-to-End 통합 플랫폼 구현
  파편화된 과정을 하나의 파이프라인으로 연결하여, 정보 탐색부터 실전 대비까지 단일 플랫폼 내에서 해결하도록 아키텍처를 설계함 1. 서비스 간 경계 제거 (Unified Pipeline) : Django와 AWS RDS 인프라를 기반으로 '입시 정보 챗봇'과 '비자 모의 인터뷰'를 통합하여, 대학 탐색 후 즉시 비자 준비로 이어지는 심리스(Seamless)한 사용자 경험을 제공함 2. 고비용 구조의 기술적 해결 : 특정 기관에 종속된 폐쇄적이고 고비용인 유학 정보 시장의 한계를 기술로 극복했습니다. STT, LLM, RAG 등 고도화된 AI 기술을 통해 누구나 저비용으로 고품질의 컨설팅 서비스를 누릴 수 있는 환경을 실현함 3. 데이터 기반 실전 대응력 강화 : 전 세계 대학의 방대한 모집 요강을 구조화하고 실제 영사와의 인터뷰 환경을 재현한 모듈을 구축하여, 유학 준비의 막연함을 실질적인 실전 대응력으로 전환함
...
| **이름** | **회고 내용**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| :------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 권민세   | 이번 프로젝트에서 기억에 남는 점은 FastAPI를 단순한 API 서버가 아니라, Django 서비스와 AI 모델 실행 환경을 연결하는 경험이었다. Django가 사용자 흐름과 화면을 담당하고, FastAPI가 RunPod 환경에서 LLM 호출, STT, TTS, 평가 로직을 처리해보는 뜻깊은 경험이였다.어려웠던 부분은 STT/TTS를 단순 기능으로 붙이는 것이 아니라, 실제 인터뷰 흐름 안에서 자연스럽게 동작하도록 연결하는 과정이었다. 사용자의 음성 입력이 녹음, 전송, 변환, 평가, 다음 질문 생성으로 이어져야 했기 때문에 요청과 응답의 상태 관리가 중요했다. 특히 로컬 환경과 RunPod 환경의 포트, 경로, 모델 위치, 패키지 설치 상태가 달라 같은 코드라도 실행 환경에 따라 달랐기 때문에 어려웠다. |
...
| 정재훈   | 기존의 딥러닝 모델 OR LLM 챗봇을 적용하여 홈페이지의 페이지만 구현으로 끝냈던 것을 백엔드&프론트엔드를 활용하여 적용할 수 있었다. 이를 통해 전반적인 논리적인 프로세스를 이해할 수 있었던 계기가 되었다.시간을 갈아넣어서 서비스의 기획부터 끝까지 수행해본 것이 너무 행복했고 향후, 이러한 경험은 가장 큰 자산이 될 것 같아서 뿌듯했다. 또한 Fastapi, AWS, Docker를 모두 사용하여 서버를 연결해본 것도 향후 큰 무기가 될 수 있을 것 같다.                                                                                                                                                                                                                  |
| 조아름   | 프로젝트 경험이 조금 있기는 하지만 요구사항 정의서와 같은 문서들을 이번처럼 자세하게 작성해본 적이 없었는데 생각보다 오랜 시간이 걸리고 머리도 너무 아팠지만 확실히 구현하기 전에 구체적으로 정리를 하니까 구현을 더 수월하고 빠르게 할 수 있었던 것 같다. 기획 단계에서 이러한 것들이 얼마나 중요한지 깨닫게 되었고 중요한만큼 이번 경험이 정말 소중했다고 생각한다. 그리고 이번에 django도 처음 써보고 AWS도 익숙하지 않아서 웹개발에 생각보다 시간을 많이 쓰게 되어서 Ai 모델을 많이 못만져봐서 아쉬웠다. 다음 프로젝트에서는 AI 모델쪽을 더 집중적으로 다루어보고싶다.                                                                                  |
