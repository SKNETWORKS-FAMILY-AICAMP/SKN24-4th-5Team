...
| 음성 자연스러움  | 높음    | 높음         | 매우 높음     | 높음        | 보통       | 매우 높음      |
| Python 연동 | ✔     | ✔          | ✔         | ✔         | ✔        | ✔          |
| API 비용    | 없음    | 유료         | 유료        | 없음        | 없음       | 유료         |
| **응답 속도** | 빠름    | 매우 빠름      | 매우 빠름     | 중간        | 빠름 | 빠름         |
...
| 권민세 | 이번 프로젝트에서 기억에 남는 점은 FastAPI를 단순한 API 서버가 아니라, Django 서비스와 AI 모델 실행 환경을 연결하는 경험이었다. Django가 사용자 흐름과 화면을 담당하고, FastAPI가 RunPod 환경에서 LLM 호출, STT, TTS, 평가 로직을 처리해보는 뜻깊은 경험이였다.
어려웠던 부분은 STT/TTS를 단순 기능으로 붙이는 것이 아니라, 실제 인터뷰 흐름 안에서 자연스럽게 동작하도록 연결하는 과정이었다. 사용자의 음성 입력이 녹음, 전송, 변환, 평가, 다음 질문 생성으로 이어져야 했기 때문에 요청과 응답의 상태 관리가 중요했다. 특히 로컬 환경과 RunPod 환경의 포트, 경로, 모델 위치, 패키지 설치 상태가 달라 같은 코드라도 실행 환경에 따라 달랐기 때문에 어려웠다. |

| 박정은 | EC2에 FastAPI를 배포하고 RunPod GPU 서버 및 RDS와 연결하여 핵심 기능을 구현했으나, 각 서비스 간 보안 그룹(Security Group) 및 외부 접근 권한 설정 과정에서 복잡한 네트워크 병목이 발생했습니다. 다음 단계에서는 AWS VPC 내에 인프라를 체계적으로 배치하고, 환경 변수(.env)를 통해 엔드포인트 관리를 일원화하여 연결 효율성과 보안성을 동시에 강화해야 합니다 |

| 정재훈 | 기존의 딥러닝 모델 OR LLM 챗봇을 적용하여 홈페이지의 페이지만 구현으로 끝냈던 것을 백엔드&프론트엔드를 활용하여 적용할 수 있었다. 이를 통해 전반적인 논리적인 프로세스를 이해할 수 있었던 계기가 되었다.
 시간을 갈아넣어서 서비스의 기획부터 끝까지 수행해본 것이 너무 행복했고 향후, 이러한 경험은 가장 큰 자산이 될 것 같아서 뿌듯했다. 또한 Fastapi, AWS, Docker를 모두 사용하여 서버를 연결해본 것도 향후 큰 무기가 될 수 있을 것 같다. |
